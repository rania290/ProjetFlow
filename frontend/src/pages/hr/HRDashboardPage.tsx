...
export const HRDashboardPage = () => {
  const { user } = useAuth();
  const { data: leaves, isLoading } = useLeaveRequests(user?.id);
  ...
  return (
    <AppLayout title="Tableau de Bord RH" subtitle="Résumé de vos activités RH">
      <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading ? (
            ...
            <>
              <StatCard label="Total" value={total} colorClass="bg-pink-50 text-pink-600" borderClass="border-slate-100"
                icon={<ClipboardList className="h-6 w-6" />} />
              <StatCard label="En Attente" value={pending} colorClass="bg-amber-50 text-amber-600" borderClass="border-slate-100"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 rounded-[2.5rem] bg-white p-8 border border-slate-200 shadow-sm relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-pink-50/50 blur-3xl rounded-full group-hover:scale-125 transition-transform duration-700" />
            <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-6 flex items-center gap-2">
              <LayoutDashboard className="w-4 h-4 text-pink-500" /> Calendrier des absences
            </h2>
            {isLoading ? (
          ...
          <div className="lg:col-span-2 rounded-[2.5rem] bg-white p-8 border border-slate-200 shadow-sm relative overflow-hidden group">
            <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-6 flex items-center gap-2">
              <Clock className="w-4 h-4 text-pink-500" /> Activité récente
            </h2>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center p-20 border-2 border-dashed border-slate-100 rounded-[2rem]">
                ...
                <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Aucune activité pour le moment</p>
              </div>
            ) : (
              <div className="space-y-4 relative z-10">
                {recent.map(l => (
                  <div key={l.id} className="flex items-center gap-5 rounded-[1.5rem] border border-slate-100 bg-slate-50/30 p-5 hover:bg-white hover:border-pink-100 hover:shadow-xl hover:shadow-pink-50/20 transition-all duration-300">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                        {new Date(l.createdAt).toLocaleDateString('fr-FR')} • {l.durationDays} jour(s)
                      </span>
                    </div>
                    <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${l.status === 'APPROVED' ? 'bg-emerald-500 text-white'
                      ...
                      {l.status === 'APPROVED' ? 'Validé' : l.status === 'REJECTED' ? 'Rejeté' : 'En attente'}
                    </div>
                  </div>
                ))}
              </div>
