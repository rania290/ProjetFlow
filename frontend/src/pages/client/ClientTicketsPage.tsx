...
import {
    ...
    MessageSquare, Paperclip, Send, X, Check, MoreHorizontal
} from 'lucide-react';
...
const STATUS_CONFIG: Record<TicketStatus, { label: string; color: string; bg: string; dot: string; icon: React.ReactNode }> = {
    ...
    WAITING_ON_CLIENT: { label: 'En attente', color: 'text-violet-700', bg: 'bg-violet-50', dot: 'bg-violet-500', icon: <MessageSquare className="w-3.5 h-3.5" /> },
...
export const ClientTicketsPage: React.FC = () => {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            showNotification(`Fichier "${file.name}" prêt à être envoyé !`);
    ...
    return (
        <AppLayout title="Support" subtitle="Gérez vos demandes d'assistance">
            <AnimatePresence>
                {notification && (
                    <motion.div
                        ...
                        className="fixed top-20 left-1/2 z-[100] px-6 py-3 bg-slate-900 text-white rounded-2xl shadow-2xl flex items-center gap-3 border border-white/10 backdrop-blur-md"
                    >
            ...
            <div className="flex flex-1 overflow-hidden bg-[#f8fafc]" style={{ height: 'calc(100vh - 57px)' }}>
                {/* Lise Column */}
                <div className={`flex flex-col transition-all duration-300 ${selectedTicket ? 'w-1/2 border-r border-slate-200' : 'w-full'} p-6`}>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-xl shadow-sm transition-colors transform hover:scale-105"
                        >
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between text-[11px] font-medium text-slate-500 pt-3 border-t border-slate-100/80">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span>{ticket.updatedAt || ticket.createdAt ? new Date(ticket.updatedAt || ticket.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }) : '—'}</span>
                                        </div>
                                    </div>
                                </div>
                            );
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 custom-scrollbar">
                                {selectedTicket.messages.map(msg => (
                                    <div key={msg.id} className={`flex gap-3 max-w-[85%] ${msg.isClient ? 'ml-auto flex-row-reverse' : ''}`}>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 shadow-sm ${msg.isClient ? 'bg-primary-600 text-white' : 'bg-slate-200 text-slate-700'}`}>
                                        ...
                                        <div>
                                            ...
                                            <div className={`p-3.5 text-sm leading-relaxed ${msg.isClient ? 'bg-primary-600 rounded-2xl rounded-tr-none text-white shadow-md' : 'bg-white rounded-2xl rounded-tl-none border border-slate-200 text-slate-700 shadow-sm'}`}>
                                                {msg.content}
                                            </div>
                                        </div>
                                    </div>
