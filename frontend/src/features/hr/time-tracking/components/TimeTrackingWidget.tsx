...
type TimeTrackingWidgetProps = {
  showClock?: boolean;
  showHistory?: boolean;
  /** When true, only the recent history list is shown (full width below the main row). */
...
export const TimeTrackingWidget: React.FC<TimeTrackingWidgetProps> = ({
  ...
  if (showHistoryOnly) {
    return <div className={className}>{historySection}</div>;
  }

  return (
    <div className={`space-y-6 ${className}`}>
      {showClock && (
      <Card className="p-6 h-full min-h-[320px] border-none shadow-xl bg-gradient-to-br from-slate-900 to-slate-800 text-white overflow-hidden relative group flex flex-col">
