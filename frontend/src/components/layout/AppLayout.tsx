...
import {
    ...
    ArrowLeftRight, Briefcase, UserCog, HeartPulse, ClipboardList, Network, Clock
...
const NAV_ITEMS = [
    ...
    { id: 'pointage', label: 'Pointage', icon: <Clock className="w-4 h-4 text-emerald-500" />, path: '/hr/pointage' },
...
export const HR_NAV_ITEMS = [
    ...
    { id: 'hr-validations', label: 'Validations', icon: <CheckSquare className="w-4 h-4" />, path: '/hr/validations', roles: ['SUPER_ADMIN', 'ADMIN', 'HR_ADMIN', 'PROJECT_MANAGER', 'MANAGER'] },
    { id: 'hr-hierarchy', label: 'Organigramme', icon: <Network className="w-4 h-4" />, path: '/hr/hierarchy' },
    { id: 'hr-pointage', label: 'Pointage & Temps', icon: <Clock className="w-4 h-4" />, path: '/hr/pointage' },
                ...
                {state.sidebarOpen && (
                    ...
                    >
                        ...
                        {(isAdmin || isClientPortal || isHRPortal) && (
                            <div className={`px-4 py-3 flex items-center justify-center gap-2 border-b border-white/5 ${isAdmin ? 'bg-emerald-500/10' : isHRPortal ? 'bg-amber-700/10' : 'bg-green-500/10'}`}>
                                <div className={`w-2 h-2 rounded-full ${isAdmin ? 'bg-emerald-500' : isHRPortal ? 'bg-amber-600' : 'bg-green-500'} animate-pulse shadow-sm`} />
                                <span className={`text-[11px] font-bold uppercase tracking-widest ${isAdmin ? 'text-emerald-400' : isHRPortal ? 'text-amber-400' : 'text-green-400'}`}>
                        ...
                        <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5">
                            ...
                            {isAdmin ? (
                                ...
                                <>
                                    <div className="px-2 py-1.5 mb-1 text-[10px] font-bold text-amber-400/80 uppercase tracking-[0.2em]">
                                        Espace RH
                                    </div>
                                    {HR_NAV_ITEMS.filter(item => !item.roles || (user?.role && item.roles.includes(user.role))).map(item => {
                                        ...
                                        return (
                                            <Link
                                                ...
                                                style={isActive ? {
                                                    ...
                                                    borderLeft: '2px solid rgb(180,83,9)'
                                                } : { borderLeft: '2px solid transparent' }}
                                            >
                                                <span className={isActive ? 'text-amber-400' : 'text-white/30 group-hover:text-white/60'}>
                                                    {item.icon}
                                                </span>
                                                <span className="flex-1">{item.label}</span>
                                            </Link>
                                    ...
                                    <div className="pt-4">
                                        <Link
                                            to="/dashboard"
                                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-amber-300/80 hover:text-amber-200 transition-all border border-amber-700/20 hover:border-amber-600/40 hover:bg-amber-700/10"
                                        >
                                            <ArrowLeftRight className="w-4 h-4 flex-shrink-0" />
                                ...
                                <>
                                    ...
                                    <div className="pt-3">
                                        <div 
                                            onClick={toggleOpen}
                                            className="p-3 rounded-xl border cursor-pointer hover:bg-indigo-500/10 transition-colors" 
                                        ...
                                        >
                                            ...
                                            <p className="text-[10px] text-white/40 leading-relaxed">
                                                {state.projects.length === 0
                                                    ...
                                                    : 'Cliquez ici pour discuter avec Aura de votre projet actif.'}
                                            </p>
                                        </div>
                                    </div>
